...
!`${CLAUDE_PLUGIN_ROOT}/skills/tile/tile.js --relay --layouts 2>&1`
...
| `--save NAME` | also save this command as layout NAME; `--add NAME` appends a further step to it |
| `--run NAME` | run every step of a saved layout, in order (`--dry-run` applies to all) |
...
1. **Resolve the request against the inventory.** If the request is empty, ask what they want and stop.
   ...
   - **Saved layouts.** If the request names a saved layout from the list above ("my dev layout", "the usual", the name itself) → the plan is just `--run NAME`. If the request asks to save, remember or name the arrangement ("save this as dev", "remember this layout") → append `--save NAME` to the first command and `--add NAME` to each further one; take NAME from the request, or propose a short one and say so.
   - **Ambiguity → show both.** When a phrase has two reasonable readings — a "quadrant" on an ultrawide (left quarter of the width vs the top-left corner), "on the left" with two monitors (left monitor vs left half), a count that might mean the front N or all — do not silently pick one. Print both commands as **A** and **B**, dry-run both, show both `Layout:` lines, say which you recommend and why (window sizes usually decide it), and ask which to apply.
2. **Print the plan** — one line per command, exactly as the user would type it, so they can copy it:
   ...
   For a saved layout: `/macos:tile --run dev`. When saving: `/macos:tile Terminal --front 2 --region left --save dev`.
...
4. **Apply only with consent.** Ask "Apply?" (or "Apply A or B?") and wait — unless the request itself says to apply ("apply", "do it", "go", "now"), in which case apply immediately. To apply, run each command without `--dry-run`, in order, and report each `Tiled … of …` summary line; mention any line marked ✗ with where the window settled.
