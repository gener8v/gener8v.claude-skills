...
Do not fix anything. A sweep that edits code has stopped being a sweep. The plugin's hooks refuse a write outside `.gener8v/` other than a test file, and send you back if a shell command changed source. Your only write is the sweep report (and, if you wrote a failing test as proof, that test file — say so in the report).
