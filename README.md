...
skills/
  ...
  orchestrate/scripts/pipeline-context.sh   # State script output injected into Orchestrate
...
- `allowed-tools` only where the user types the command (Setup, Delivery) or where it pays for itself (Orchestrate, whose injected state needs it). A skill with `allowed-tools` must itself be approved whenever Claude invokes it on its own, so the model-invoked skills pre-approve nothing
...
- **Writing a skill:**
  ...
  - `allowed-tools` has a price on a skill Claude invokes itself: the invocation then needs approval — a prompt in an interactive session, a denial in a headless or scheduled one. Use it on user-invoked skills (`disable-model-invocation: true`), and on a model-invoked skill only when it saves more prompts than it costs; document the one-time approval and the `Skill(gener8v:<name>)` allow rule when you do. The routing evals grant the Skill tool, so they cannot catch this — check with `claude -p "<natural request>" --permission-mode default` in a fixture copy.
  ...
  - An injected `` !`command` `` that exits non-zero aborts the whole skill, and one that is not pre-approved does too. Inject only scripts that always exit 0 and are listed in `allowed-tools` (which brings the approval above), and make them degrade to one line under a copied install.
