...
1. **Regenerate State**: Run the deterministic generator and read its summary:
   ...
   - Tickets `changes/<change-slug>/tickets/<area-slug>.md` — read each ticket's **Priority** and **Depends On**
   ...
   - Legacy layout: top-level `tickets/`, `delivery/` and `reviews/*-review.md` are read as the pseudo-change `initial`
...
`scripts/gener8v-state.py` writes `.gener8v/pipeline-state.yaml` in this shape (schema version 3). When writing it by hand, produce the same shape. The example is the state of the Support Documentation Search System with its first change, `support-search`, in delivery.
...
changes:
  support-search:
    ...
    areas_detail:
      search-and-retrieval:
        tickets: changes/support-search/tickets/search-and-retrieval.md
        ticket_count: 4
...
- **changes**: one entry per `changes/<change-slug>/` (plus `initial` when the legacy layout is present, marked `legacy: true`). `declared_status` is what the brief says; `status` is the working status the script derives (`planned` · `ready` · `in_delivery` · `complete` · `abandoned`) and a warning names any disagreement. `areas` come from the brief's Affected Capability Areas table and from the ticket files; `pending_specification` lists areas whose Requirements cell still says pending, `pending_breakdown` areas with no ticket file yet. `deliveries` is keyed `<area-slug>/TICKET-NNN` because ticket numbering restarts per file.
...
- **warnings**: orphaned specifications, PRD without capability areas, Brownfield mid-run, legacy layout, several active changes, a change naming an area the PRD lacks, declared-status disagreements, etc.
